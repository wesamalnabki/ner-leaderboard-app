    ...
    def display_leaderboard(dataset_name, dataset_dict):
        ...
        for sub in submissions:
            ...
            if reeval_col.button("🔁", key=f"reeval_{sub.id}"):
                ...
                st.session_state['reeval_metric'] = 'micro_strict'
            ...
            if st.session_state.get("reeval_id") == sub.id:
                if not os.path.exists(file_path):
                    ...
                    default_metric = st.session_state.get(f"metric_select_{sub.id}", "micro_strict")
                    index = ["micro_strict", "macro_strict", "micro_relaxed", "macro_relaxed"].index(default_metric)

                    def update_reeval_metric(sub_id):
                        st.session_state['reeval_metric'] = st.session_state.get(f"metric_select_{sub_id}",
                                                                                 "micro_strict")
                    ...
                    selected_metric = st.selectbox(
                        ...
                        on_change=update_reeval_metric,
                        args=(sub.id,)
                    )
